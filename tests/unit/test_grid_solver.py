...
def test_validate_placement_stub(empty_grid: Grid):
    ...
    assert empty_grid.validate_placement("word", GridPosition(x=0, y=0))
...
def test_get_valid_words_mock_dict(simple_dictionary: MockDictionary):
    ...
    assert set(valid_words) == {
        "at",
        "cat",
        "act",
    }  # "act" is also a permutation of "atc"
...
def test_solve_qless_grid_simple_case(monkeypatch, simple_dictionary: MockDictionary):
    ...
    monkeypatch.setattr(
        "qless_solver.grid_solver.Dictionary", lambda: simple_dictionary
    )
